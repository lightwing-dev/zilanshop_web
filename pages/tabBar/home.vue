	...
	<view>
		...
		<view class="swiper">
			<view class="swiper-box">
				<swiper circular="true" autoplay="true" @change="swiperChange">
					<swiper-item v-for="swiper in swiperList" :key="swiper.bid">
						<image :src="CONSTANT.baseURL+swiper.imgpath" @tap="toSwiper(swiper)"></image>
					</swiper-item>
				</swiper>
		...
		<view class="category-list">
			<view class="category" v-for="(row, index) in categoryList" :key="index" @tap="toCategory(row)">
				<view class="img">
					<image :src="CONSTANT.baseURL+row.iconimgpath"></image>
				</view>
				<view class="text">{{ row.gtypename }}</view>
			</view>
		</view>
		<!-- 商品列表 -->
		<view class="goods-list">
			...
			<view class="product-list">
				<view class="product" v-for="product in productList" :key="product.gid" @tap="toGoods(product)">
			...
			<view class="product-list">
				<view class="product" v-for="product in SalesVolume" :key="product.gid" @tap="toGoods(product)">
					<image mode="widthFix" :src="CONSTANT.baseURL+product.img"></image>
					<view class="name">{{ product.gname }}</view>
					<view class="info">
						<view class="price">¥{{ product.price }}</view>
		...
		methods: {
			...
			getBannerList() {
				uni.request({
					...
					method: 'POST',
					success: (json) => {
						this.swiperList = json.data.data
					}
				});
			},
			/**
